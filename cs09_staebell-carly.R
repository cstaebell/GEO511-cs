...
# Plot min temperature and snowfall 
...
# Get traffic report data from Buffalo Open Data
open_data_endpoint <- "https://data.buffalony.gov/resource/6at3-hpb5.json"

open_data_params <- list("$where" = "report_date >= '2024-10-01' AND report_date <= '2025-03-31'", "$limit" = 5500)
...
# Spatial data like road locations could help reveal any spatial patterns in the data, 
#  such as if certain locations are more prone to accidents, and if weather is a factor at those locations.

# Poisson regression may be appropriate, since we are working with non-negative count data.
# Independence may be an issue for both linear regression and Poisson, since a disabled vehicle
# on the road from one accident may make additional accidents more likely. 
# The irregularity of weather patterns may also pose a problem for the underlying Poisson assumption of constant rate. 
